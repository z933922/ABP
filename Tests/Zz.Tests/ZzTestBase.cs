...
namespace Zz.Tests
{
    public abstract class ZzTestBase : AbpIntegratedTestBase<ZzTestModule>
    {
        ...
        protected void UsingDbContext(Action<ZzDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ZzDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ZzDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ZzDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ZzDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ZzDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ZzDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ZzDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ZzDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ZzDbContext, Task<T>> func)
        {
            T result;
